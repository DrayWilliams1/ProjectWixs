...
<?php
/** 
 * A file for checking whether a user has admin access
 * 
 * -- Additional Notes --
...
/**
 * Queries the database for the user and checks if they are an admin
 */
function checkAdmin() {
    global $pdo;
    global $email_post;
    ...
    $sql_select = "SELECT admin FROM users WHERE email = ?";
    ...
    if($stmt->execute()) { // The query has executed successfully
        ...
        if ($stmt->rowCount() > 0) {
            ...
            $responseObject['isAdmin']=$user['admin'];
            return true;
